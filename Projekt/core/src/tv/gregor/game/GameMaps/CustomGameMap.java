...
import tv.gregor.game.customgamemap.CustomGameMapData;
import tv.gregor.game.customgamemap.CustomGameMapLoader;
...
public class CustomGameMap extends GameMap {
        ...
        for(int layer = 0 ; layer < getLayers(); layer++){
            for (int row = 0; row < getHeight(); row++) {
                for (int col = 0; col < getWidth(); col++) {
                    TileType type = this.getTileTypeByCoordiante(layer, col, row);
                    if(type != null){
    ...
    public TileType getTileTypeByLocation(int layer, float x, float y) {
        return this.getTileTypeByCoordiante(layer,(int) (x / TileType.TILE_SIZE),getHeight() - (int) (y/TileType.TILE_SIZE) - 1 );
    }
